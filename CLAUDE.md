# Memória e Regras do Assistente (PS4 Linux Project)

> **Para múltiplos agentes:** veja `.claude-memory.yml` — define a estrutura unificada de memória e documentação do projeto.
...
- Este `CLAUDE.md` é o ÚNICO arquivo de memória/regras na raiz do projeto (o antigo `MEMORY.md` duplicado foi removido em 2026-07-19 por decisão do usuário — `CLAUDE.md` já é carregado automaticamente pelo Claude Code em toda sessão, então não há necessidade de um segundo arquivo).
