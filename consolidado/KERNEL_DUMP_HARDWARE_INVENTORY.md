...
| `baikal_pcie.c` / glue | `decompiled_baikal_pcie_attach.txt`, `decompiled_dc7190d0.txt`, `decompiled/baikal_glue_block_reset_dc6df.txt`, `decompiled/baikal_glue_write_dc718710.txt` | **Hold/pulse da GBE: bloco `0x2000`, hold `0x180020`, pulse `0x180074` — confirmado por RE** (a rotina de stop do MAC da GBE, `fcn.dc59fe10`, é chamada imediatamente antes desse bloco na descompilação de `fcn.ffffffffdc6df850`). ⚠️ Correção 2026-07-25: uma entrada anterior desta linha (e do `AGENTS.md`) trocou os offsets achando que o hold correto seria `0x180034` — isso vinha de inferência por padrão nunca cruzada com a descompilação, e estava errado (`0x180034` é de um bloco diferente/não identificado, `0x3c00`). Testes M7/M8 (`ICC_GBE_TEST_LOG.md`) já tinham mostrado que o registrador lê "liberado" mesmo sem escrita nossa; o teste ao vivo de 2026-07-25 com o offset correto (`0x180020`) confirmou isso — sem incidente, mas sem destravar o PHY. |
