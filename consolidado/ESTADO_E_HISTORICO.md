...
- 🔴 **HD interno SATA (`ata1`, TOSHIBA MQ04ABF100) morre ~30-45s após o boot — EM ABERTO, e custa 78s (58%) do tempo de boot.** Não é PHY, não é EFUSE, não é garbage collection do SMR, não é máscara de interrupção: `SErr 0x0` em 100% das falhas, e os registradores AHCI lidos ao vivo confirmam `GHC=0x80000002` (IE ligado), `PxIE=0x7840007f`, `PxIS=0`, `PxSERR=0`. O AHCI *compartilhava* a IRQ 32 com os dois xHCI; isso foi corrigido em `bpcie_assign_irqs()` (clamp `nvec=1` → `min(nvec, bpcie_max_vectors(func))`) e a tag `20260728-sata-irq-dedicada` deu a ele o vetor dedicado hwirq **5345** (`ata1 ... irq 33`), sem regressão em USB/`mmc0`/`mts`/Blu-ray. **Mas o HD continua falhando.** Pista atual: o contador da IRQ 33 parou em **7** contra 8.212 do USB — a entrega de interrupção cessa após o probe. Hipótese: `bpcie_handle_edge_irq()` foi escrito para um único vetor pai e, com 3 vetores, os três descritores limpam o **mesmo** registrador de ACK do glue, com o USB consumindo os bits do SATA. Próximo passo: instrumentar o demux e decidir entre não-demultiplexar ou tornar o demux seguro para multi-vetor. Detalhes em `PLANO_SATA_INTERNO_100PCT_2026-07-28.md`; memória em `memory/sata-interno-falha-e-ncq-irq-compartilhada-2026-07-28.md`; banco: `test_history` ids 63 e 64.
  - ⚠️ **Dois erros de diagnóstico já cometidos, não repetir:** (1) "o AHCI não tem handler de IRQ" — **falso**, ele se chama `xhci_aeolia[0000:00:14.7]` e não `ahci[...]`, porque `ata_host_activate()` usa `dev_driver_string(host->dev)`; (2) "a falha é específica de NCQ" — **falso**, com `noncq` ativo falha como `READ DMA` (`0xc8`, `SAct 0x0`), não-enfileirado.
  - ⚠️ **`noncq` não sai pelo bootargs** — o quirk é hardcoded em `libata-core.c:4199`.
