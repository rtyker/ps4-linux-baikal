...
### ATUALIZAÇÃO 2026-07-25: dispatcher genérico de notificação ICC encontrado (mas ainda não é o alvo final)

Busquei `/r` diretamente pela string real `icc_power.c` (`0xffffffffdcb0025b`, distinta de `if_mts.c` — ver correção de 2026-07-21 acima) e achei só **duas referências de código**, ambas próximas (`0xdc528f35`, `0xdc528fa3`), dentro da mesma função `fcn.ffffffffdc528ef0`. Decompilada com `r2ghidra`:

- É o **dispatcher genérico de notificação ICC**: recebe um evento (`arg1`), incrementa um contador de reentrância, percorre uma **lista ligada de callbacks registrados** (`*(iVar8+0x40)`), e para cada entrada não removida (`!= -1`) invoca o callback registrado: `(*piVar12[5])(piVar12[3], *(arg1+4) & 0x3f, uVar3)` — ou seja, `(contexto_do_registrante, evento_mascarado_6bits, codigo_do_evento)`.
- Isso é **infraestrutura compartilhada** (tipo `EVENTHANDLER_INVOKE` genérico do FreeBSD) — qualquer driver que se inscreva em notificações ICC passa por aqui. Não é específico da GBE, e não revela por si só o major/minor que libera o loop de espera do `attach()` da GBE.
- **Próximo alvo revisado:** achar onde `SceGbeMtsCtrl`/`SceGbeMtsPhyCtrl` **se registram** nessa lista (a chamada de "subscribe"/"register callback" dentro do `attach()` da GBE, provavelmente logo antes do loop de espera em `0xdc5a4450`) — isso revelaria o major/evento exato que a GBE aguarda. Ainda não localizado; seria o próximo passo lógico de RE nesta trilha.
...
**Consequência prática:** a proposta registrada (à época) em `GBE_ACTION_PLAN.md` seção 3 — implementar `bpcie_baikal_gbe_phy_init()` transcrevendo esta sequência e chamando de `sky2_probe()` antes de ler o chip_id — **não se sustenta como está**. Transcrever escritas MDIO para um chip que não responde é inócuo. (`sky2_probe()` é arquitetura abandonada; a sequência real hoje está implementada em `drivers_mts/mts.c:mts_phy_calibration()`, que já incorpora essa correção.)
